    ...
    def analysisSex(self, title='分析好友性别分布', friends_info=None):
        ...
        for item in friends_info.get('sex'):
            if item == 0:
                ...
                sex_infos['女'] += 1
        pie = Pie(init_opts=dict(theme='westeros', page_title=title)).add(title, data_pair=tuple(
            zip(sex_infos.keys(), sex_infos.values())), rosetype='area')
