...
class QQReports():
    def __init__(self, savedir='qqdata', username='charlespikachu'):
        client = login.Client()
        # QQ空间
        infos_return, self.session_zone = client.QQZone(reload_history=True).login(username, '微信公众号: ****', 'scanqr')
        ...
        # QQ安全中心
        _, self.session_id = client.QQId(reload_history=True).login(username, '微信公众号: ****', 'scanqr')
        self.session_id_all_cookies = requests.utils.dict_from_cookiejar(self.session_id.cookies)
        # QQ群
        _, self.session_qun = client.QQQun(reload_history=True).login(username, '微信公众号: ****', 'scanqr')
